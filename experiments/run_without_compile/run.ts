...
let init = require('protractor/built/launcher')
...
SystemJS.import('config.ts').then(fileConfig=> {
    let init = require('protractor/built/launcher').init
    let configParser = new ConfigParser();

    //First adding file config
    configParser.addConfig(fileConfig);

    //then override with args config.
    configParser.addConfig(argv)

    init(undefined, configParser.getConfig())
})
